...
#define SPIBSC_CKDLY_DEFAULT        (0x0000A504)    /* Initial value */
#define SPIBSC_CKDLY_TUNING         (0x0000A50A)    /* Shorten the data input setup time and extend the data hold time */

/* Use for setting the SPODLY register */
#define SPIBSC_SPODLY_DEFAULT       (0xA5000000)    /* Initial value */
#define SPIBSC_SPODLY_TUNING        (0xA5001111)    /* Delay the data output delay/hold/buffer-on/buffer-off time */

/* ---- include configuration parameters ---- */
#include "spibsc_config.h"

/* Check error */
#if ( (SPIBSC_CMNCR_CPHAT_ODD == SPIBSC_CPHAT_SETTING) && (SPIBSC_CMNCR_CPHAR_ODD == SPIBSC_CPHAR_SETTING) )
    /* CPHAT=1(Odd), CPHAR=0(Odd) */
    #error "CPHAT=1(Odd) and CPHAR=0(Odd)" is not supported.
#elif ( (SPIBSC_DDR_TRANS == SPIBSC_TRANS_MODE) && ( (SPIBSC_CMNCR_CPHAT_EVEN == SPIBSC_CPHAT_SETTING) && (SPIBSC_CMNCR_CPHAR_EVEN == SPIBSC_CPHAR_SETTING) ) )
/* DDR, CPHAT=0(Even), CPHAR=1(Even) */
    #error "DDR, CPHAT=1(Odd) and CPHAR=0(Odd)" is not supported.
#endif
