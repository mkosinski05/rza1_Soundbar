...
int32_t R_Ether_Read (uint32_t ch, void *buf)
{
    ...
    else
    {
        /* Need to invalidate the cache */
        {
            ...
            R_CACHE_L1_CleanInvalidLine(pVirtual_addr,p->rd1.RDL);
        }
...
*               : frame is processed. It doesnÃ¯Â¿Â½ft check whether Ethernet frame
...
int32_t R_Ether_Write (uint32_t ch, void * buf, uint32_t len)
{
    ...
    /* Need to write back the cache to physical mem */
    {
        ...
        R_CACHE_L1_CleanInvalidLine(pVirtual_addr,len);
    }
...
static void lan_reg_reset (void)
{
    volatile int32_t    j = 400;            /* Wait for BÃ†áßÑò"áß×“56 cycles */

                                            /*  ((IÃ†áßÑò"áß×BÃ†áßÑò"áß×Š*256)/6cyc = 8*256/6 = 342 */
...
static void lan_reg_set (int32_t link)
{
    ...
    /* ==== EthernetÃ…ÁþÃ¢áà¬Å¾Ã¢â£æŽÃ¡Ã¥ñŽ¦Ã¯Â¿Â½Ã…Â¾Ã¢áà¬Å¡Ã¦ê^½Ã¬ã~ÑÏÃ†áßÑòAáà¬ÅÅò%áßÑÉÃ†áßÑòAáà¬Â°Ã¢â£æŽÃÅÐÃ‹Å“^ ==== */
    R_INTC_RegistIntFunc(INTC_ID_ETHERI, INT_Ether); 
    
    /* ==== EthernetÃ…ÁþÃ¢áà¬Å¾Ã¢â£æŽÃ¡Ã¥ñŽ¦Ã¯Â¿Â½Ã…Â¾Ã¢áà¬Å¡Ã¦ê^½Ã¢â£æßàÙ¥Ã¯Â¿Â½ÃƒÂ¦Ã¢áà¬ÅÅÙ=6Ã¯Â¿Â½Ãƒï¿½Ã¢â£æßä¢Ã¥ñŽ¦ ==== */
